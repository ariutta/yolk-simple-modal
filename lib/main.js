  ...
  */
  ...
  modalComponent.view = function(ctrl, args) {
    ...
    if (!!content) {
      ...
      ctrl.vm.status('loading');
      // If no content is provided, then the content is still loading,
      // so we set status to loading and show a loading icon.
      content = m('div', {
        style: {
  ...
  function close() {
    modalComponent.vm.status('closed');
    // m.redraw();
    // TODO m.redraw() above does not cause the config and view sections to run here. Why?
    // Until that's figured out, I'm using the kludge below, but this isn't ideal, because
    // it seems this should be handled by code in the config section.
    // Could it be related to compose()?
    var modalContainer = document.querySelector('.simple-modal-holder');
    modalContainer.style.display = 'none';
  }
